...
class LaboProb1(ParsingClass1):
    ...
    def factorielle(self, n: int) -> int:
        ...
        Returns :
            (long) : La valeur de (n!)
        """
        if n > self.args.fact_end:   # Correction :  > au lieu de >=
    ...
    def long_calcul(self) -> int:
        ...
        while a > self.args.val_end:
            res += a
            a -= 1  # Correction : a-=1 plutôt que : a -= self.val_end

        return res

    def compare_strings(self):
        """Comparaison de chaînes de caractères utf-8 :
           - Deux chaînes de caractères sont en apparence identiques (lorsqu'on les utilise dans un print()),
        ...
        """
        self.word1 = self.normalize_string(self.word1)  # Ajout de la normalisation des chaînes de caractères
        self.word2 = self.normalize_string(self.word2)  # Ajout de la normalisation des chaînes de caractères
    ...
    def exceptions(self) -> None:
        ...
        try:
            x = a / b
        except ZeroDivisionError:  # Correction : type d'exception adéquat (plutôt que AssertionError)
            print("Problème dans la division de a/b: a =", a, ", b =", b)
            self.print_debug_info()

        try:
            print("y = ", y)
        except NameError:  # Correction : type d'exception adéquat (plutôt que AssertionError)
            print("Variable y non définie")
            self.print_debug_info()

        try:
            print("y = ", a + c)
        except TypeError:  # Correction : type d'exception adéquat (plutôt que AssertionError)
            print("Erreur de type: impossible d'additionner un entier et une chaîne de caractères")
            self.print_debug_info()

        try:
            print("Liste[5]", ma_liste[5])
        except IndexError:  # Correction : type d'exception adéquat (plutôt que AssertionError)
        ...
        try:
            ...
            mon_fichier = open(filename, "r")
            print(mon_fichier.read())
        except FileNotFoundError:  # Correction : type d'exception adéquat (plutôt que AssertionError)
...
def main() -> None:
    """Démarrage de l'exercice 1 du labo :
        ...
        - Utiliser le paramètre -p (1, 2, 3 ou 4) pour déterminer quel problème observer
        - Utiliser le paramètre -e de concert avec -p 4 pour démarrer le traitement des exceptions inclus (mais à corriger)
    ...
    match p1.args.p:
        ...
        case 2:
            # p1.prepare() # Correction : cet appel efface p1.config, qui est nécessaire pour p1.times2()
